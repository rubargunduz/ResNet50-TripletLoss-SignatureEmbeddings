...
def verify_signature():
    ...
    if not data or "pdf_url" not in data or "signature_url" not in data:
        return jsonify({"error": "Missing 'pdf_url' or 'signature_url' in request body"}), 400
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not pdf_crops:
                return jsonify({"message": "No signatures found in the provided document"}), 200
        ...
        return jsonify({"error": f"Failed to fetch file from URL: {e}"}), 400
